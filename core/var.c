...
{
    strcpy(out->name, src);
    if (strchr(src, DOT) || strchr(src, 'e') || strchr(src, 'E')) {
        out->type = var_double;
...
void var_delete(var_list **head, const char *name)
{
    if (!(*head))
        return;
    if (!strcmp((*head)->variable.name, name)) {
        var_list *to_delete = *head;
        *head = (*head)->next;
        free(to_delete);
        return;
    }
    ...
    for (prev = (*head); prev->next; prev = prev->next) {
        curr = prev->next;
        if (!strcmp(curr->variable.name, name)) {
            prev->next = curr->next;
            free(curr);
            return;
        }
